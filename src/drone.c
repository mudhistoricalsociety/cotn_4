...
void do_abstract(CHAR_DATA * ch, char *argument)
{
        ...
        {
                xprintf(buf,
                        " #G- - - - - - - - --------========#Y{ #0Drone Realities #Y}#G========-------- - - - - - - - -#n\n\r");
                send_to_char(buf, ch);
                send_to_char("\n\r", ch);
                xprintf(buf, "  #PHorrid Realities #R[");
                for (i = 0; i < 3; i++)
                {
                        if (ch->pcdata->powers[DRONE_HORRID_REALITY] > i)
                                strcat(buf, "#0*");
                        else
                                strcat(buf, " ");
                }
                ...
                for (i = 0; i < 3; i++)
                {
                        if (ch->pcdata->powers[DRONE_DISTORTED_REALITY] > i)
                                strcat(buf, "#0*");
                        else
                                strcat(buf, " ");
                }
                ...
                for (i = 0; i < 3; i++)
                {
                        if (ch->pcdata->powers[DRONE_ALTERNATE_REALITY] > i)
                                strcat(buf, "#0*");
                        else
                                strcat(buf, " ");
                }
                ...
                xprintf(buf,
                        " #G- - - - - - - - --------========#Y{ #0Drone Psychics  #Y}#G========-------- - - - - - - - -#n\n\r");
                send_to_char(buf, ch);
                send_to_char("\n\r", ch);
                xprintf(buf, "  #PWillpower#R        [");
                for (i = 0; i < 3; i++)
                {
                        if (ch->pcdata->powers[DRONE_WILLPOWER] > i)
                                strcat(buf, "#0*");
                        else
                                strcat(buf, " ");
                }
                ...
                for (i = 0; i < 3; i++)
                {
                        if (ch->pcdata->powers[DRONE_MINDCONTROL] > i)
                                strcat(buf, "#0*");
                        else
                                strcat(buf, " ");
                }
                ...
                for (i = 0; i < 3; i++)
                {
                        if (ch->pcdata->powers[DRONE_PSIONICS] > i)
                                strcat(buf, "#0*");
                        else
                                strcat(buf, " ");
                }
                ...
                xprintf(buf,
                        " #G- - - - - - - - --------========#Y{ #0Drone Combat    #Y}#G========-------- - - - - - - - -#n\n\r");
                send_to_char(buf, ch);
                send_to_char("\n\r", ch);
                xprintf(buf, "  #PEvasion#R          [");
                for (i = 0; i < 3; i++)
                {
                        if (ch->pcdata->powers[DRONE_EVASION] > i)
                                strcat(buf, "#0*");
                        else
                                strcat(buf, " ");
                }
                ...
                for (i = 0; i < 3; i++)
                {
                        if (ch->pcdata->powers[DRONE_FORTITUDE] > i)
                                strcat(buf, "#0*");
                        else
                                strcat(buf, " ");
                }
                ...
                for (i = 0; i < 3; i++)
                {
                        if (ch->pcdata->powers[DRONE_FOCUS] > i)
                                strcat(buf, "#0*");
                        else
                                strcat(buf, " ");
                }
                ...
                send_to_char(buf, ch);
                send_to_char
                        ("                   #PSyntax #0:#G abstract #G<#0reality#G/#0psyche#G/#0combat#G> #G<#0type#G>#n\n\r",
                         ch);
                send_to_char(buf, ch);
        }
        else if (!str_cmp(arg1, "reality"))
        {
                if (!str_cmp(arg2, "horrid"))
                {
                        ...
                        {
                                send_to_char
                                        ("You have mastered this reality already.\n\r",
                                         ch);
                                return;
                        }
                        if (ch->exp <
                            cost * (ch->pcdata->powers[DRONE_HORRID_REALITY] +
                                    1))
                        {
                                xprintf(buf,
                                        "You need %ld more exp to buy this power.\n\r",
                                        cost *
                                        (ch->pcdata->
                                         powers[DRONE_HORRID_REALITY] + 1) -
                                        ch->exp);
                                send_to_char(buf, ch);
                                return;
                        }
                ...
                else if (!str_cmp(arg2, "distorted"))
                {
                        ...
                        if (ch->exp <
                            cost *
                            (ch->pcdata->powers[DRONE_DISTORTED_REALITY] + 1))
                        {
                                xprintf(buf,
                                        "You need %ld more exp to buy this power.\n\r",
                                        cost *
                                        (ch->pcdata->
                                         powers[DRONE_DISTORTED_REALITY] +
                                         1) - ch->exp);
                                send_to_char(buf, ch);
                                return;
                        }
                ...
                else if (!str_cmp(arg2, "alternate"))
                {
                        ...
                        if (ch->exp <
                            cost *
                            (ch->pcdata->powers[DRONE_ALTERNATE_REALITY] + 1))
                        {
                                xprintf(buf,
                                        "You need %ld more exp to buy this power.\n\r",
                                        cost *
                                        (ch->pcdata->
                                         powers[DRONE_ALTERNATE_REALITY] +
                                         1) - ch->exp);
                                send_to_char(buf, ch);
                                return;
                        }
        ...
        else if (!str_cmp(arg1, "psyche"))
        {
                if (!str_cmp(arg2, "willpower"))
                {
                        ...
                        {
                                send_to_char
                                        ("You have mastered this psyche already.\n\r",
                                         ch);
                                return;
                        }
                        if (ch->exp <
                            cost * (ch->pcdata->powers[DRONE_WILLPOWER] + 1))
                        {
                                xprintf(buf,
                                        "You need %ld more exp to buy this power.\n\r",
                                        cost *
                ...
                else if (!str_cmp(arg2, "mindcontrol"))
                {
                        ...
                        if (ch->exp <
                            cost * (ch->pcdata->powers[DRONE_MINDCONTROL] +
                                    1))
                        {
                                xprintf(buf,
                                        "You need %ld more exp to buy this power.\n\r",
                ...
                else if (!str_cmp(arg2, "psionics"))
                {
                        ...
                        if (ch->exp <
                            cost * (ch->pcdata->powers[DRONE_PSIONICS] + 1))
                        {
                                xprintf(buf,
                                        "You need %ld more exp to buy this power.\n\r",
                                        cost *
        ...
        else if (!str_cmp(arg1, "combat"))
        {
                if (!str_cmp(arg2, "evasion"))
                {
                        ...
                        {
                                send_to_char
                                        ("You have mastered this art of combat already.\n\r",
                                         ch);
                                return;
                        }
                        if (ch->exp <
                            cost * (ch->pcdata->powers[DRONE_EVASION] + 1))
                        {
                                xprintf(buf,
                                        "You need %ld more exp to buy this power.\n\r",
                                        cost *
                ...
                else if (!str_cmp(arg2, "fortitude"))
                {
                        ...
                        if (ch->exp <
                            cost * (ch->pcdata->powers[DRONE_FORTITUDE] + 1))
                        {
                                xprintf(buf,
                                        "You need %ld more exp to buy this power.\n\r",
                                        cost *
                ...
                else if (!str_cmp(arg2, "focus"))
                {
                        ...
                        if (ch->exp <
                            cost * (ch->pcdata->powers[DRONE_FOCUS] + 1))
                        {
                                xprintf(buf,
                                        "You need %ld more exp to buy this power.\n\r",
                                        cost *
