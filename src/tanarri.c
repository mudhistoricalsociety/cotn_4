...
void do_taneq(CHAR_DATA * ch, char *argument)
{
        ...
        {
                send_to_char
                        ("Please specify which piece of tanar'ri armor you wish to make: Claymore Bracer\n\r"
        ...
        {
                send_to_char
                        ("It costs 100 points of primal to create Tanar'ri equipment.\n\r",
                         ch);
                return;
        }
...
void do_chaossurge(CHAR_DATA * ch, char *argument)
{
        ...
        if (victim->alignment > 500)
                ...
                dam = 1000;
        xprintf(buf, "You let chaos envelope $N and $E is hurt [%d]", dam);
        act(buf, ch, NULL, victim, TO_CHAR);
        xprintf(buf,
                "$n points a wicked finger at you and chaos strikes your mind! [%d]",
                dam);
        act(buf, ch, NULL, victim, TO_VICT);
        xprintf(buf, "$n strikes $N down with a burst of pure #RCHAOS!#n.");
...
void do_enmity(CHAR_DATA * ch, char *argument)
{
        ...
        if (number_percent() > 60)
        {
                ...
                send_to_char
                        ("#RSomeone is trying to control your actions!!!#n\n\r",
                         victim1);
        if (number_percent() > 60)
        {
...
void do_bloodsac(CHAR_DATA * ch, char *argument)
{
        ...
        {
                send_to_char
                        ("You have gained all powers for your current status.\n\r",
                         ch);
                return;
        }
        ...
        {
                if (!IS_SET
                    (ch->pcdata->powers[TANARRI_POWER], TANARRI_TRUESIGHT))
                {
                        send_to_char
                                ("You now have the power of truesight.\n\r",
                                 ch);
                        SET_BIT(ch->pcdata->powers[TANARRI_POWER],
                                TANARRI_TRUESIGHT);
                }
                else if (!IS_SET
                         (ch->pcdata->powers[TANARRI_POWER], TANARRI_CLAWS))
                {
                        send_to_char
                                ("You now have the ability to grow claws.\n\r",
                                 ch);
                        SET_BIT(ch->pcdata->powers[TANARRI_POWER],
                                TANARRI_CLAWS);
                }
                ...
                {
                        send_to_char
                                ("You now have the power to summon an earthquake.\n\r",
                                 ch);
                        SET_BIT(ch->pcdata->powers[TANARRI_POWER],
                                TANARRI_EARTHQUAKE);
                }
                ...
                {
                        send_to_char
                                ("Your char is bugged, note an immortal.\n\r",
                                 ch);
                        return;
                }
        }
        ...
        {
                if (!IS_SET
                    (ch->pcdata->powers[TANARRI_POWER], TANARRI_EXOSKELETON))
                {
                        send_to_char
                                ("Your entire body is made from massive bones.\n\r",
                                 ch);
                        SET_BIT(ch->pcdata->powers[TANARRI_POWER],
                                TANARRI_EXOSKELETON);
                }
                else if (!IS_SET
                         (ch->pcdata->powers[TANARRI_POWER], TANARRI_FANGS))
                {
                        send_to_char
                                ("long pointy fangs slides out your gums.\n\r",
                                 ch);
                        SET_BIT(ch->pcdata->powers[TANARRI_POWER],
                                TANARRI_FANGS);
                }
                else if (!IS_SET
                         (ch->pcdata->powers[TANARRI_POWER], TANARRI_TORNADO))
                {
                        send_to_char
                                ("You now have the power to summon a tornado storm.\n\r",
                                 ch);
                        SET_BIT(ch->pcdata->powers[TANARRI_POWER],
                                TANARRI_TORNADO);
                }
        ...
        if (ch->pcdata->rank == 3)
        {
                if (!IS_SET(ch->pcdata->powers[TANARRI_POWER], TANARRI_SPEED))
                {
                        ...
                        SET_BIT(ch->pcdata->powers[TANARRI_POWER],
                         ...
                         (ch->pcdata->powers[TANARRI_POWER], TANARRI_MIGHT))
                {
        ...
        if (ch->pcdata->rank == 4)
        {
                ...
                {
                        send_to_char
                                ("You skin turns red hot, and your body becomes enveloped in flames.\n\r",
                                 ch);
                        SET_BIT(ch->pcdata->powers[TANARRI_POWER],
                                TANARRI_FIERY);
                }
                else if (!IS_SET
                         (ch->pcdata->powers[TANARRI_POWER], TANARRI_FURY))
                {
                        send_to_char
                                ("You now have the power to let your fury take over.\n\r",
                                 ch);
                        SET_BIT(ch->pcdata->powers[TANARRI_POWER],
                                TANARRI_FURY);
                }
                ...
                {
                        send_to_char
                                ("An extra head grows from your neck.\n\r",
                                 ch);
                        SET_BIT(ch->pcdata->powers[TANARRI_POWER],
                                TANARRI_HEAD);
                }
        ...
        {
                if (!IS_SET
                    (ch->pcdata->powers[TANARRI_POWER], TANARRI_BOOMING))
                {
                        send_to_char
                                ("Your voice carries the power of a legion.\n\r",
                                 ch);
                        SET_BIT(ch->pcdata->powers[TANARRI_POWER],
                                TANARRI_BOOMING);
                }
                else if (!IS_SET
                         (ch->pcdata->powers[TANARRI_POWER], TANARRI_ENRAGE))
                {
                        send_to_char
                                ("You have learned the power to enrage your opponents.\n\r",
                                 ch);
                        SET_BIT(ch->pcdata->powers[TANARRI_POWER],
                                TANARRI_ENRAGE);
                }
                else if (!IS_SET
                         (ch->pcdata->powers[TANARRI_POWER], TANARRI_FLAMES))
                {
                        send_to_char
                                ("You can call upon the flames from the abyss at will.\n\r",
                                 ch);
                        SET_BIT(ch->pcdata->powers[TANARRI_POWER],
                                TANARRI_FLAMES);
                }
        ...
        {
                if (!IS_SET
                    (ch->pcdata->powers[TANARRI_POWER], TANARRI_TENDRILS))
                {
                        send_to_char
                                ("You grow tendrils from every part of your body.\n\r",
                                 ch);
                        SET_BIT(ch->pcdata->powers[TANARRI_POWER],
                                TANARRI_TENDRILS);
                }
                else if (!IS_SET
                         (ch->pcdata->powers[TANARRI_POWER], TANARRI_LAVA))
                {
                        ...
                        SET_BIT(ch->pcdata->powers[TANARRI_POWER],
                         ...
                         (ch->pcdata->powers[TANARRI_POWER], TANARRI_EMNITY))
                {
                        send_to_char
                                ("You now have the power to cause emnity between people.\n\r",
                                 ch);
                        SET_BIT(ch->pcdata->powers[TANARRI_POWER],
                                TANARRI_EMNITY);
                }
...
void do_lavablast(CHAR_DATA * ch, char *argument)
{
        ...
        {
                send_to_char
                        ("Your not up to it, you ain't got the fire in ya.\n\r",
                         ch);
                return;
        }
...
void do_booming(CHAR_DATA * ch, char *argument)
{
        ...
        else
        {
                ...
                {
                        send_to_char
                                ("Your booming voice stuns your opponent.\n\r",
                                 ch);
                        send_to_char
                                ("The pain is to much, and you drop to the ground.\n\r",
                                 victim);
...
void do_earthquake(CHAR_DATA * ch, char *argument)
{
        ...
        {
                send_to_char
                        ("You need more mana to summon an earthquake.\n\r",
                         ch);
                return;
        }
...
void do_infernal(CHAR_DATA * ch, char *argument)
{
        ...
        if (ch->mana < 2000)
        {
                if (IS_CLASS(ch, CLASS_TANARRI))
                        send_to_char
                                ("You need more mana to summon the infernal flames of the abyss.\n\r",
                                 ch);
                else
        ...
        if (IS_CLASS(ch, CLASS_TANARRI))
                send_to_char
                        ("You summon the infernal flames of the abyss.\n\r",
                         ch);
        else
                send_to_char
                        ("You mutter an incantation and a fireball flies from your hands.\n\r",
                         ch);
        ich_next = ch->in_room->people;
        ich = ich_next;
        while (ich_next != NULL)
