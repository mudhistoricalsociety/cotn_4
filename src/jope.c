...
void jope_newbits(CHAR_DATA * ch, char *argument)
{
        ...
        if (arg[0] == '\0')
        {
                ...
                for (;;)
                {
                        iCount = 0;
                        xprintf(buf, "  ");
                        while (newbits_table[cmd].name[0] != '\0'
                               && iCount < 5)
                        {
                                xprintf(buf2, "%-12s ",
                                        newbits_table[cmd].name);
                                strcat(buf, buf2);
                                cmd++;
                                iCount++;
                        }
        ...
        {
                if (arg[0] == newbits_table[cmd].name[0]
                    && !str_prefix(arg, newbits_table[cmd].name))
                {
                        found = TRUE;
                        break;
                }
        ...
        if (IS_SET(victim->newbits, newbits_table[cmd].bit))
                ...
                SET_BIT(victim->newbits, newbits_table[cmd].bit);
        send_to_char("Ok.\n\r", ch);
        return;
}
...
void jope_act(CHAR_DATA * ch, char *argument)
{
        ...
        if (arg[0] == '\0')
        {
                ...
                for (;;)
                {
                        ...
                        {
                                xprintf(buf2, "%-12s ", act_table[cmd].name);
                                strcat(buf, buf2);
                                cmd++;
                                iCount++;
                        }
        ...
        {
                if (arg[0] == act_table[cmd].name[0]
                    && !str_prefix(arg, act_table[cmd].name))
                {
                        found = TRUE;
                        break;
                }
        ...
        if (IS_SET(victim->act, act_table[cmd].bit))
                ...
                SET_BIT(victim->act, act_table[cmd].bit);
        send_to_char("Ok.\n\r", ch);
        return;
}
...
void jope_jflags(CHAR_DATA * ch, char *argument)
{
        ...
        if (arg[0] == '\0')
        {
                ...
                for (;;)
                {
                        iCount = 0;
                        xprintf(buf, "  ");
                        while (jflags_table[cmd].name[0] != '\0'
                               && iCount < 5)
                        {
                                xprintf(buf2, "%-12s ",
                                        jflags_table[cmd].name);
                                strcat(buf, buf2);
                                cmd++;
                                iCount++;
                        }
        ...
        {
                if (arg[0] == jflags_table[cmd].name[0]
                    && !str_prefix(arg, jflags_table[cmd].name))
                {
                        found = TRUE;
                        break;
                }
        ...
        if (IS_SET(victim->pcdata->jflags, jflags_table[cmd].bit))
                ...
                SET_BIT(victim->pcdata->jflags, jflags_table[cmd].bit);
        send_to_char("Ok.\n\r", ch);
        return;
}
...
void jope_extra(CHAR_DATA * ch, char *argument)
{
        ...
        {
                if (arg[0] == extra_table[cmd].name[0]
                    && !str_prefix(arg, extra_table[cmd].name))
                {
                        found = TRUE;
                        break;
                }
        ...
        if (IS_SET(victim->extra, extra_table[cmd].bit))
                ...
                SET_BIT(victim->extra, extra_table[cmd].bit);
        send_to_char("Ok.\n\r", ch);
        return;
}
...
void jope_stances(CHAR_DATA * ch, char *argument)
{
        ...
        {
                send_to_char
                        ("What do you wish to set the stance to (a number) ?\n\r",
                         ch);
                return;
        }
        value = atoi(arg2);
  ...
  else if (!str_prefix(arg1, "pitbull"))
  ...
  else if (!str_prefix(arg1, "hawk"))
    victim->stance[STANCE_HAWK] = value;
  else if (!str_prefix(arg1, "badger"))
    victim->stance[STANCE_BADGER] = value;
  else if (!str_prefix(arg1, "cougar"))
    victim->stance[STANCE_COUGAR] = value;
  else if (!str_prefix(arg1, "wolverine"))
...
void jope_weapons(CHAR_DATA * ch, char *argument)
{
        ...
        {
                send_to_char
                        ("What do you wish to set the weapon to (a number) ?\n\r",
                         ch);
                return;
        }
        value = atoi(arg2);
...
void jope_spells(CHAR_DATA * ch, char *argument)
{
        ...
        {
                send_to_char
                        ("What do you wish to set the spell to (a number) ?\n\r",
                         ch);
                return;
        }
        value = atoi(arg2);
...
void jope_list(CHAR_DATA * ch, char *argument)
{
        ...
        for (;;)
        {
                ...
                while (jope_table[cmd].name[0] != '\0' && iCount < 5)
                {
                        ...
                        xprintf(buf2, "%-12s ", jope_table[cmd].name);
                        strcat(buf, buf2);
                        cmd++;
                        iCount++;
                }
...
void jope_show(CHAR_DATA * ch, char *argument)
{
        ...
        xprintf(buf,
                "  Exp : %ld   Qps : %d   Primal : %d   Level : %d   Trust : %d\n\r",
                victim->exp, victim->pcdata->quest, victim->practice,
                victim->level, victim->trust);
        send_to_char(buf, ch);
        xprintf(buf, "  Hit : %d/%d   Move : %d/%d   Mana : %d/%d\n\r",
                victim->hit, victim->max_hit, victim->move, victim->max_move,
                victim->mana, victim->max_mana);
        send_to_char(buf, ch);
        xprintf(buf,
                "  Pkill : %d   Pdeath : %d   Mkill : %d   Mdeath : %d\n\r",
                victim->pkill, victim->pdeath, victim->mkill, victim->mdeath);
        send_to_char(buf, ch);
        send_to_char(line, ch);
        xprintf(buf,
                "  Purple : %3d  Red    : %3d  Blue   : %3d  Green  : %3d  Yellow : %3d\n\r",
                victim->spl[PURPLE_MAGIC], victim->spl[RED_MAGIC],
                victim->spl[BLUE_MAGIC], victim->spl[GREEN_MAGIC],
                victim->spl[YELLOW_MAGIC]);
        send_to_char(buf, ch);
        send_to_char(line, ch);
        xprintf(buf,
                "  Viper  : %3d  Crane  : %3d  Crab   : %3d  Mong   : %3d  PitBull   : %3d\n\r",
                victim->stance[STANCE_VIPER], victim->stance[STANCE_CRANE],
                victim->stance[STANCE_CRAB], victim->stance[STANCE_MONGOOSE],
                victim->stance[STANCE_PITBULL]);
        send_to_char(buf, ch);
        xprintf(buf,
                "  Hawk : %3d  Badger : %3d  Cougar  : %3d  Wolver : %3d  Griz  : %3d\n\r",
                victim->stance[STANCE_HAWK], victim->stance[STANCE_BADGER],
                victim->stance[STANCE_COUGAR], victim->stance[STANCE_WOLVERINE],
                victim->stance[STANCE_GRIZZLY]);
        send_to_char(buf, ch);
        send_to_char(line, ch);
        xprintf(buf,
                "  Unarm : %4d  Slice : %4d  Stab  : %4d  Slash : %4d  Whip  : %4d\n\r",
                victim->wpn[0], victim->wpn[1], victim->wpn[2],
                victim->wpn[3], victim->wpn[4]);
        send_to_char(buf, ch);
        xprintf(buf,
                "  Claw  : %4d  Blast : %4d  Pound : %4d  Crush : %4d  Grep  : %4d\n\r",
                victim->wpn[5], victim->wpn[6], victim->wpn[7],
                victim->wpn[8], victim->wpn[9]);
        send_to_char(buf, ch);
        xprintf(buf, "  Bite  : %4d  Pierc : %4d  Suck  : %4d\n\r",
                victim->wpn[10], victim->wpn[11], victim->wpn[12]);
        send_to_char(buf, ch);
        send_to_char(line, ch);

        xprintf(buf, "  Act     : %s\n\r",
                flag_string(act_table, victim->act));
        send_to_char(buf, ch);
        xprintf(buf, "  Newbits : %s\n\r",
                flag_string(newbits_table, victim->newbits));
        send_to_char(buf, ch);
        xprintf(buf, "  Jflags  : %s\n\r",
                flag_string(jflags_table, victim->pcdata->jflags));
        send_to_char(buf, ch);
        xprintf(buf, "  Extra   : %s\n\r",
                flag_string(extra_table, victim->extra));
        send_to_char(buf, ch);
...
void do_pfile(CHAR_DATA * ch, char *argument)
{
        ...
        {
                send_to_char
                        ("You cannot edit pfiles of a higher level than your own.\n\r",
                         ch);
                jope_free_char(ch);
                ch->desc->connected = CON_PLAYING;
        }
