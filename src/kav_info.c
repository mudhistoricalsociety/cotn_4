...
void skillstance(CHAR_DATA * ch, CHAR_DATA * victim)
{
        ...
        if (victim->stance[stance] <= 0)
                xprintf(bufskill, "completely unskilled in");
                ...
                xprintf(bufskill, "a grand master of");
        else
                return;
        if (stance == STANCE_VIPER)
                ...
                xprintf(stancename, "bull");
        else if (stance == STANCE_MANTIS)
                xprintf(stancename, "mantis");
        else if (stance == STANCE_DRAGON)
                xprintf(stancename, "dragon");
        else if (stance == STANCE_TIGER)
                xprintf(stancename, "tiger");
        else if (stance == STANCE_MONKEY)
                xprintf(stancename, "monkey");
        else if (stance == STANCE_SWALLOW)
                xprintf(stancename, "swallow");
        else if (stance == STANCE_WOLF)
                xprintf(stancename, "wolf");
        else
                return;
        if (ch == victim)
                xprintf(buf, "You are %s the %s stance.", bufskill,
                        stancename);
        else
                xprintf(buf, "$N is %s the %s stance.", bufskill, stancename);
...
void show_spell(CHAR_DATA * ch, int dtype)
{
        ...
        if (ch->spl[dtype] == 00)
                xprintf(bufskill, "untrained at");
        else if (ch->spl[dtype] <= 25)
                xprintf(bufskill, "an apprentice at");
        else if (ch->spl[dtype] <= 50)
                xprintf(bufskill, "a student at");
                ...
                xprintf(bufskill, "a lich master of");
                ...
                return;
        if (dtype == 0)
                xprintf(buf, "You are %s purple magic.\n\r", bufskill);
        else if (dtype == 1)
                xprintf(buf, "You are %s red magic.\n\r", bufskill);
        else if (dtype == 2)
                xprintf(buf, "You are %s blue magic.\n\r", bufskill);
        else if (dtype == 3)
                xprintf(buf, "You are %s green magic.\n\r", bufskill);
        else if (dtype == 4)
                xprintf(buf, "You are %s yellow magic.\n\r", bufskill);
        else
                return;
        send_to_char(buf, ch);
        return;
        ...
        for (vch = char_list; vch != NULL; vch = vch_next)
        {
                ...
                if (vch->in_room == ch->in_room)
                {
                        if (!IS_NPC(vch) && !IS_AFFECTED(vch, AFF_POLYMORPH))
                                xprintf(buf, "   %s #w(#RPlayer#w)#n\n\r",
                                        vch->name);
                        else if (!IS_NPC(vch)
                                 && IS_AFFECTED(vch, AFF_POLYMORPH))
                                xprintf(buf, "   %s #w(#RPlayer#w)#n\n\r",
                                        vch->morph);
...
void birth_date(CHAR_DATA * ch, bool is_self)
{
        ...
        if (mm == 1)
                xprintf(mstatus, "month");
        else
                xprintf(mstatus, "months");
        if (dd == 1)
                xprintf(dstatus, "day");
        else
                xprintf(dstatus, "days");
        if (is_self)
        {
                if (!IS_EXTRA(ch, EXTRA_BORN))
                        yy += 17;
                xprintf(buf,
                        "  You are #C%d #nyears #C%d#n %s and #C%d #n%s old\n\r",
        ...
        {
                if (yy > 0 || (dd > 0 && mm >= 9)
                    || IS_EXTRA(ch, EXTRA_LABOUR))
                {
                        if (!IS_EXTRA(ch, EXTRA_LABOUR))
                                SET_BIT(ch->extra, EXTRA_LABOUR);
                        xprintf(buf, "#RYou are ready to give birth.#n\n\r");
                        send_to_char(buf, ch);
                }
                else if (yy > 0 || mm > 0)
                {
                        xprintf(buf, "You are %d %s and %d %s pregnant.\n\r",
...
void other_age(CHAR_DATA * ch, int extra, bool is_preg, char *argument)
{
        ...
        if (!is_preg)
        {
                if (!IS_EXTRA(ch, EXTRA_BORN))
                        yy += 17;
                xprintf(buf, "#GAge: #W%d #Gyears, #W%d #G%s and #W%d #G%s#n\n\r",
                        yy, mm, mstatus, dd, dstatus);
                send_to_char(buf, ch);
        }
        else
        {
                if (yy > 0 || (dd > 0 && mm >= 9)
                    || IS_SET(extra, EXTRA_LABOUR))
                {
                        xprintf(buf, "She is ready to give birth.\n\r");
                        send_to_char(buf, ch);
                }
                else if (yy > 0 || mm > 0)
                {
                        xprintf(buf, "She is %d %s and %d %s pregnant.\n\r",
                                mm, mstatus, dd, dstatus);
                        send_to_char(buf, ch);
                }
        }
...
void do_consent(CHAR_DATA * ch, char *argument)
{
        ...
        act("You give $N permission to FUCK on you.", ch, NULL, victim,
            TO_CHAR);
        act("$n has given you permission to FUCK on $m.", ch, NULL, victim,
            TO_VICT);
        return;
}
