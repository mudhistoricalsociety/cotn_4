...
void do_vote(CHAR_DATA * ch, char *argument)
{
        ...
        if (arg1[0] == '\0')
        {
                if (vote[0])
                {
                        xprintf(buf, "The topic of the vote is [%s]\n\r"
...
//          xprintf(buf+strlen(buf),"[#w%s{x] Yes[{Y%.2f%%{x] No[#R%.2f%%{x] Total Votes[%d]\n\r",
        ...
        if (IS_IMMORTAL(ch))
        {
                if (!str_cmp(arg1, "cancel"))
                {
                        ...
                        xprintf(buf,
                                "Info-> The vote has been canceled by %s.\n\r",
                                ch->name);
                        do_echo(ch, buf);
                        return;
                }
                ...
                if (!str_cmp(arg1, "player"))
                {
                        if (!str_cmp(arg2, "on"))
                        {
                                player_vote = 1;
                                send_to_char
                                        ("Players may start a vote topic now.\n\r",
                                         ch);
                                return;
                        }
                        ...
                        if (!str_cmp(arg2, "off"))
                        {
                                player_vote = 0;
                                send_to_char
                                        ("Players may not start a vote topic now.\n\r",
                                         ch);
                                return;
                        }
...
void vote_update(void)
{
        ...
        if (vote[0] <= 0)
        {
                ...
                xprintf(buf, "#wVote has ended for topic #D[#W%s#D]#n\n\r",
                        vote_topic);
                do_info(NULL, buf);
                xprintf(buf,
                        "#wVote outcome is #GYes#D[#Y%.2f%%#D] #RNo#D[#Y%.2f%%#D] #BTotal Votes#D[#Y%d#D]#n\n\r",
                        x1, x2, (int) total);
                do_info(NULL, buf);
