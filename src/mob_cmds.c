...
void do_mpstat(CHAR_DATA * ch, char *argument)
{
...

        {
                xprintf(arg,
                        ...
                        ++i, prog_type_to_name(mprg->trig_type), mprg->vnum,
                        mprg->trig_phrase);
                send_to_char(arg, ch);
        }
...
void do_mpcall(CHAR_DATA * ch, char *argument)
{
        ...
        //extern void program_flow( int, char *, CHAR_DATA *, OBJ_DATA *, ROOM_INDEX_DATA *, CHAR_DATA *, void *, void *,int );
...
void do_mprandsoc(CHAR_DATA * ch, char *argument)
{
        ...
        randnum = number_range(0, iSocial - 1);

        if (argument[0] == '\0')
        {
                act(social_table[randnum].others_no_arg, ch, NULL, victim,
                    TO_ROOM);
                act(social_table[randnum].char_no_arg, ch, NULL, victim,
                    TO_CHAR);
        }
        ...
        {
                act(social_table[randnum].others_auto, ch, NULL, victim,
                    TO_ROOM);
                act(social_table[randnum].char_auto, ch, NULL, victim,
                    TO_CHAR);
        }
        else
        {
                act(social_table[randnum].others_found, ch, NULL, victim,
                    TO_NOTVICT);
                act(social_table[randnum].char_found, ch, NULL, victim,
                    TO_CHAR);
                act(social_table[randnum].vict_found, ch, NULL, victim,
                    TO_VICT);
                    ...
                    && IS_AWAKE(victim) && victim->desc == NULL)
                {
                        ...
                        case 7:
                        case 8:
                                act(social_table[randnum].others_found,
                                    victim, NULL, ch, TO_NOTVICT);
                                act(social_table[randnum].char_found,
                                    victim, NULL, ch, TO_CHAR);
                                act(social_table[randnum].vict_found,
...
void do_opstat(CHAR_DATA * ch, char *argument)
{
...

        {
                xprintf(arg,
                        "[%2d] Trigger [%-8s] Program [%4d] Phrase [%s]\n\r",
                        ++i, prog_type_to_name(oprg->trig_type), oprg->vnum,
                        oprg->trig_phrase);
                send_to_char(arg, ch);
        }
...
void do_opechoat(OBJ_DATA * obj, char *argument)
{
        ...
        act(argument,
            obj->carried_by ? obj->carried_by : obj->in_room->people, obj,
            victim, TO_VICT);
}
...
void do_opecho(OBJ_DATA * obj, char *argument)
{
        ...
        act(argument,
            obj->carried_by ? obj->carried_by : obj->in_room->people, NULL,
            NULL, TO_ROOM);
        act(argument,
            obj->carried_by ? obj->carried_by : obj->in_room->people, NULL,
            NULL, TO_CHAR);
                            ...
                            in_room, arg)) == NULL)
        {
                                  ...
                                  carried_by->in_room, arg)))
                {
                        extract_obj(vobj);
                         ...
                         && (vobj =
                             get_obj_carry(obj->carried_by, arg,
                                           NULL)) != NULL)
                {
                        extract_obj(vobj);
                }
                         ...
                         && (vobj =
                             get_obj_wear(obj->carried_by, arg,
                                          FALSE)) != NULL)
                {
...
void do_opcall(OBJ_DATA * obj, char *argument)
{
        ...
        argument = one_argument(argument, arg);
        if (arg[0] != '\0')
                vch = get_char_room2(NULL,
                                     obj->in_room ? obj->in_room : obj->
                                     carried_by->in_room, arg);
        argument = one_argument(argument, arg);
        if (arg[0] != '\0')
                obj1 = get_obj_here(NULL,
                                    obj->in_room ? obj->in_room : obj->
                                    carried_by->in_room, arg);
        argument = one_argument(argument, arg);
        if (arg[0] != '\0')
                obj2 = get_obj_here(NULL,
                                    obj->in_room ? obj->in_room : obj->
                                    carried_by->in_room, arg);
        program_flow(prg->vnum, prg->code, NULL, obj, NULL, vch,
                     (void *) obj1, (void *) obj2, 1);
}
...
{
        CHAR_DATA *ch;
        char      target[MAX_INPUT_LENGTH], arg1[MAX_INPUT_LENGTH],
        ...
        else if (!isdigit(arg1[0]))
        {
                ...
                case '/':
                        ...
                        break;
                default:
                        bug("OpAttrib: invalid modifier from vnum %d",
                            obj->pIndexData->vnum);
                        return;
                }
        }
...
void do_rpstat(CHAR_DATA * ch, char *argument)
{
...

        {
                xprintf(arg,
                        "[%2d] Trigger [%-8s] Program [%4d] Phrase [%s]\n\r",
                        ++i, prog_type_to_name(rprg->trig_type), rprg->vnum,
                        rprg->trig_phrase);
                send_to_char(arg, ch);
        }
...
void do_rpasound(ROOM_INDEX_DATA * room, char *argument)
{
        ...
        for (door = 0; door < 6; door++)
        {
                ...
                {
                        act(argument, pexit->u1.to_room->people, NULL, NULL,
                            TO_ROOM);
                        act(argument, pexit->u1.to_room->people, NULL, NULL,
                            TO_CHAR);
                }
...
void do_rppurge(ROOM_INDEX_DATA * room, char *argument)
{
        ...
        if ((victim = get_char_room2(NULL, room, arg)) == NULL)
        {
                ...
                {
                        bug("Rppurge - Bad argument from vnum %d.",
                            room->vnum);
                }
                return;
        }
...
void do_rptransfer(ROOM_INDEX_DATA * room, char *argument)
{
        ...
        else
        {
                ...
                {
                        bug("Rptransfer - No such location from vnum %d.",
                            room->vnum);
                        return;
                }
...
void do_rpcall(ROOM_INDEX_DATA * room, char *argument)
{
        ...
        program_flow(prg->vnum, prg->code, NULL, NULL, room, vch,
                     (void *) obj1, (void *) obj2, 1);
}
...
void do_rpotransfer(ROOM_INDEX_DATA * room, char *argument)
{
        ...
        if (arg[0] == '\0')
        {
                bug("RpOTransfer - Missing argument from vnum %d.",
                    room->vnum);
                return;
        }
        one_argument(argument, buf);
        ...
        {
                bug("RpOTransfer - No such location from vnum %d.",
                    room->vnum);
                return;
        }
