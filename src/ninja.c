...
void do_principles(CHAR_DATA * ch, char *argument)
{
        ...
        {
                xprintf(buf,
                        "Principles: Sora (%d), Chikyu (%d), Ningenno (%d).\n\r",
        ...
        if (arg2[0] == '\0')
        {
                if (!str_cmp(arg1, "sora"))
                {
                        ...
                        if (ch->pcdata->powers[NPOWER_SORA] < 1)
                                send_to_char
                                        ("You have none of the Sora principles.\n\r",
                                         ch);
                        ...
                        if (ch->pcdata->powers[NPOWER_SORA] > 4)
                                send_to_char
                                        ("Kanzuite -Aware- The truesight power.\n\r",
                                         ch);
                ...
                {
                        send_to_char
                                ("Chikyu - Preperation for battle.\n\r\n\r",
                                 ch);
                        if (ch->pcdata->powers[NPOWER_CHIKYU] < 1)
                                send_to_char
                                        ("You have none of the Chikyu principles.\n\r",
                                         ch);
                        if (ch->pcdata->powers[NPOWER_CHIKYU] > 0)
                                send_to_char
                                        ("Tsuyoku -Strength- Toughness.\n\r",
                                         ch);
                        if (ch->pcdata->powers[NPOWER_CHIKYU] > 1)
                                send_to_char
                                        ("Songai -Damage- Enhanced damage.\n\r",
                                         ch);
                        if (ch->pcdata->powers[NPOWER_CHIKYU] > 2)
                                send_to_char
                                        ("Isogu -Haste- Adds two extra attacks.\n\r",
                                         ch);
                        ...
                        if (ch->pcdata->powers[NPOWER_CHIKYU] > 4)
                                send_to_char
                                        ("Sakeru -Sonic Speed- Dodge more attacks.\n\r",
                                         ch);
                ...
                else if (!str_cmp(arg1, "ningenno"))
                {
                        ...
                        if (ch->pcdata->powers[NPOWER_NINGENNO] < 1)
                                send_to_char
                                        ("You have none of the Ningenno principles.\n\r",
                                         ch);
        ...
        if (!str_cmp(arg2, "improve"))
        {
                ...
                {
                        send_to_char
                                ("Principles: Sora, Chikyu, Ningenno.\n\r",
                                 ch);
                        return;
                }
                ...
                ch->practice -= cost;
                xprintf(buf,
                        "You improve your ability in the %s principle.\n\r",
                        arg1);
                send_to_char(buf, ch);
        }
        else
...
void do_michi(CHAR_DATA * ch, char *argument)
{
        ...
        if (IS_CLASS(ch, CLASS_NINJA))
        {
                ...
                {
                        send_to_char
                                ("But you are already in the state of Michi.\n\r",
                                 ch);
                        return;
                }
                if (ch->move < 500)
                {
                        send_to_char
                                ("But you don't have enough move to perform the michi.\n\r",
                                 ch);
                        return;
                }
...
void do_kakusu(CHAR_DATA * ch, char *argument)
{
        ...
        {
                send_to_char
                        ("You have not learned the Sora principle to 3.\n\r",
                         ch);
                return;
        }
...
void do_kanzuite(CHAR_DATA * ch, char *argument)
{
        ...
        {
                send_to_char
                        ("You have not learned the Sora principle to 5.\n\r",
                         ch);
                return;
        }
        ...
        {
                send_to_char
                        ("You don't have 500 move to increase your awareness.\n\r",
                         ch);
                return;
        }
        ...
        else
        {
                ...
                send_to_char
                        ("You're senses increase into incredible proportions.\n\r",
                         ch);
                ch->move -= 500;
        }
...
void do_mienaku(CHAR_DATA * ch, char *argument)
{
        ...
        {
                send_to_char
                        ("You have not learned the Ningenno principle to 3.\n\r",
                         ch);
                return;
        }
        ...
        {
                send_to_char
                        ("You don't have enough movement points to flee.\n\r",
                         ch);
                return;
        }
...
void do_bomuzite(CHAR_DATA * ch, char *argument)
{
        ...
        {
                send_to_char
                        ("You have not learned the Sora principle to 6.\n\r",
                         ch);
                return;
        }
...
void do_hara_kiri(CHAR_DATA * ch, char *argument)
{
        ...
        {
                send_to_char
                        ("You are already experiencing the power of HaraKiri.\n\r",
                         ch);
                return;
        }
...
void do_ninjaarmor(CHAR_DATA * ch, char *argument)
{
        ...
        {
                send_to_char
                        ("Please specify which piece of ninja eq you wish to make: Ring Collar Robe Cap "
...
void do_strangle(CHAR_DATA * ch, char *argument)
{
        ...
        {
                send_to_char
                        ("They are to hurt, you cannot get near enough.\n\r",
                         ch);
                return;
        }
        if (is_safe(ch, victim))
                return;
        act("$n stabs $N in the back, causing great damage.", ch, NULL,
            victim, TO_NOTVICT);
