...
void do_spiderarmor(CHAR_DATA * ch, char *argument)
{
        ...
        {
                send_to_char
                        ("Please specify which piece of spider armor you wish to make: whip ring\n\r"
        ...
        {
                send_to_char
                        ("It costs 150 points of practice to create a piece of spider armour.\n\r",
                         ch);
                return;
        }
...
void do_battlestation(CHAR_DATA * ch, char *argument)
{
        ...
        if (!IS_SET
            (ch->pcdata->powers[DROID_IMPLANTS], IMPLANT_BATTLESTATION))
        {
...
void do_creload(CHAR_DATA * ch, char *argument)
{
        ...
        {
                send_to_char
                        ("What do you wish to reload : ammo or rockets ?\n\r",
                         ch);
                return;
        }
        if (!str_cmp(arg, "ammo"))
        {
                ...
                if ((!IS_SET
                     (ch->pcdata->powers[DROID_IMPLANTS], IMPLANT_ARMORY)
                     && ch->pcdata->powers[DROID_AMMO] >= 2)
        ...
        else if (!str_cmp(arg, "rockets"))
        {
                ...
                if ((!IS_SET
                     (ch->pcdata->powers[DROID_IMPLANTS], IMPLANT_ARMORY)
                     && ch->pcdata->powers[DROID_ROCKET] >= 1)
...
void do_dimplant(CHAR_DATA * ch, char *argument)
{
        ...
        {
                send_to_char
                        ("                 #p{{{#0-#p}}} #0Spiderdroid Implants #p{{{#0-#p}}}#n\n\r\n\r",
                         ch);
                send_to_char
                        (" #PEyes         #0[#p 25000#0]  #PExtra Arms      #0[#p100000#0]  #PRadar         #0[#p100000#0]#n\n\r",
                         ch);
                send_to_char
                        (" #PMachinegun   #0[#p200000#0]  #PRocket Launcher #0[#p150000#0]  #PBody Armor    #0[#p100000#0]#n\n\r",
                         ch);
                send_to_char
                        (" #PArmwires     #0[#p200000#0]  #P360 Vision      #0[#p 50000#0]  #PArmory        #0[#p200000#0]#n\n\r",
                         ch);
                send_to_char
                        (" #PSteam Engine #0[#p150000#0]  #PBreath Cannon   #0[#p100000#0]  #PBattlestation #0[#p400000#0]#n\n\r",
                         ch);
                send_to_char
                        (" #PShadow Gate  #0[#p100000#0]  #PTuned Engine    #0[#p100000#0]  #PRegrowth      #0[#p 25000#0]#n\n\r",
                         ch);
        ...
        {
                send_to_char
                        ("You do not have enough droid points to buy this implant.\n\r",
                         ch);
                return;
        }
        if (ch->exp < 500 * cost)
        {
                send_to_char
                        ("You do not have enough exp to buy this implant.\n\r",
                         ch);
                return;
        }
        ch->exp -= cost * 500;
...
void do_droidspeed(CHAR_DATA * ch, char *argument)
{
        ...
        send_to_char
                ("You feel a rush of power as your fighting speed picks up.\n\r",
                 ch);
...
void do_submachine(CHAR_DATA * ch, char *argument)
{
        ...
        if (!IS_SET
            (ch->pcdata->powers[DROID_IMPLANTS], IMPLANT_SUBMACHINEGUN))
        {
                send_to_char("You don't have that implant.\n\r", ch);
                return;
        }
        ...
        {
                send_to_char
                        ("You are blessed by Lloth and fire of another volley.\n\r",
                         ch);
                one_hit(ch, victim, gsn_machinegun, 1);
                one_hit(ch, victim, gsn_machinegun, 1);
        }
...
void do_rocketblast(CHAR_DATA * ch, char *argument)
{
        ...
        if (!IS_SET
            (ch->pcdata->powers[DROID_IMPLANTS], IMPLANT_ROCKETLAUNCHER))
        {
                send_to_char("You don't have that implant.\n\r", ch);
                return;
        }
