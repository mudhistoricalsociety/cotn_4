...
void do_monkarmor(CHAR_DATA * ch, char *argument)
{
        ...
        {
                send_to_char
                        ("Please specify which piece of monk armor you wish to make: Ring Collar Robe\n\r"
        ...
        {
                send_to_char
                        ("It costs 60 points of primal to create monk equipment.\n\r",
                         ch);
                return;
        }
...
void do_deathtouch(CHAR_DATA * ch, char *argument)
{
        ...
        act("$n places $s hands on your head, and you scream in utter pain.",
            ch, NULL, victim, TO_VICT);
        act("$n places $s hands on $N's head and $N screams in pain.", ch,
            NULL, victim, TO_ROOM);
...
void do_healingtouch(CHAR_DATA * ch, char *argument)
{
        ...
        act("You place your hands on $N's head and focus your energy on $M.",
            ch, NULL, victim, TO_CHAR);
        act("$n places $s hands on your head and you feel warmer.", ch, NULL,
            victim, TO_VICT);
        act("$n places $s hands on $N's head and concentrates.", ch, NULL,
            victim, TO_ROOM);
...
void do_chi(CHAR_DATA * ch, char *argument)
{
        ...
        if (ch->chi[CURRENT] >= ch->chi[MAXIMUM])
        {
                ...
                {
                        xprintf(buf,
                                "You cannot focus your ch'i past a level of %d.",
                                ch->chi[MAXIMUM]);
                        stc(buf, ch);
                }
                return;
        ...
        if (ch->chi[CURRENT] == 0)
        {
                ...
                act("$n's body flickers with energy.", ch, NULL, NULL,
                    TO_ROOM);
        }
        if (ch->chi[CURRENT] == 1)
        {
        ...
        if (ch->chi[CURRENT] == 2)
        {
                ...
                act("$n's body glows with blue energy.", ch, NULL, NULL,
                    TO_ROOM);
        }
        if (ch->chi[CURRENT] == 3)
        {
        ...
        if (ch->chi[CURRENT] == 5)
        {
                ...
                act("$n's body emits sparks of energy.", ch, NULL, NULL,
                    TO_ROOM);
        }
        WAIT_STATE(ch, 12);
        ch->chi[CURRENT]++;
...
void do_chands(CHAR_DATA * ch, char *argument)
{
        ...
        if (IS_ITEMAFF(ch, ITEMA_CHAOSHANDS))
        {
                ...
                act("The swirling colors around your hands disappear.", ch,
                    NULL, NULL, TO_CHAR);
                act("The swirling colors around $n's hands disappear.", ch,
                    NULL, NULL, TO_ROOM);
                return;
        }
        else
        {
                ...
                act("Swirling colors appear around $n's hands.", ch, NULL,
                    NULL, TO_ROOM);
                return;
        }
}
...
void do_mantra(CHAR_DATA * ch, char *argument)
{
        ...
        if (arg1[0] == '\0' && arg2[0] == '\0')
        {
                ...
                if (ch->pcdata->powers[PMONK] >= 14)
                        send_to_char
                                ("You have completed your Mantra training.\n\r",
                                 ch);
                return;
        }
        if (!str_cmp(arg2, "improve"))
        {
                ...
                {
                        xprintf(buf,
                                "You have already gained all the known Mantras.\n\r");
                        send_to_char(buf, ch);
                        return;
                }
                if (cost > ch->practice)
                {
                        xprintf(buf,
                                "It costs you %d primal learn the Mantra.\n\r",
                                cost);
                        send_to_char(buf, ch);
                        return;
                }
                ...
                ch->practice -= cost;
                xprintf(buf, "You learn a Mantra.\n\r");
                send_to_char(buf, ch);
        }
        else
                send_to_char
                        ("To learn a Mantra, type: Mantra power improve.\n\r",
                         ch);
        return;
}
...
void do_cloak(CHAR_DATA * ch, char *argument)
{
        ...
        {
                send_to_char
                        ("You do not have the mantra of the Cloak of Life.\n\r",
                         ch);
                return;
        }
        if (ch->move < 1000)
        {
                send_to_char
                        ("You do not have enough vitality to do that.\n\r",
                         ch);
                return;
        }
        if (IS_SET(ch->newbits, NEW_MONKCLOAK))
        {
                ...
                send_to_char
                        ("You are no longer protected by the Almighty's cloak.\n\r",
                         ch);
...
void do_prayofages(CHAR_DATA * ch, char *argument)
{
        ...
        {
                send_to_char
                        ("You must obtain level 11 in mantras to use Prayer of Ages.\n\r",
                         ch);
                return;
        }
        ...
        send_to_char
                ("You pray to god, cursing your nemesis with his divine wrath.\n\r",
                 ch);
        WAIT_STATE(ch, 8);
        return;
}
...
void do_sacredinvis(CHAR_DATA * ch, char *argument)
{
        ...
        {
                send_to_char
                        ("You do not have the mantra of Sacred Invisibility.\n\r",
                         ch);
                return;
        }
        if (IS_SET(ch->act, AFF_HIDE))
        {
                ...
                send_to_char
                        ("You emerge from your cloak of invisibility.\n\r",
                         ch);
                act("$n slowly emerges from a cloak of holy invisibility.",
                    ch, NULL, NULL, TO_ROOM);
        }
        else
        {
                ...
                {
                        send_to_char
                                ("Not until your fight timer expires.\n\r",
                                 ch);
                        return;
                }
                if (ch->move < 500)
                {
                        send_to_char
                                ("You do not have enough movement to do this.\n\r",
                                 ch);
                        return;
                }
                send_to_char
                        ("You slowly fade into a cloak of invisibility.\n\r",
                         ch);
                act("$n slowly fades into a cloak of holy invisibility..", ch,
                    NULL, NULL, TO_ROOM);
...
void do_flaminghands(CHAR_DATA * ch, char *argument)
{
        ...
        {
                send_to_char
                        ("You do not have the mantra of Flaming Hands.\n\r",
                         ch);
                return;
        }
...
void do_celestial(CHAR_DATA * ch, char *argument)
{
        ...
        {
                xprintf(buf,
                        "You concentrate and open a Celestial path to %s!\n\r",
                        victim->short_descr);
                send_to_char(buf, ch);
        }
        if (!IS_NPC(victim))
...
void do_godseye(CHAR_DATA * ch, char *argument)
{
        ...
        {
                send_to_char
                        ("You have not learned the Eyes of God Mantra.\n\r",
                         ch);
                return;
        }
        ...
        {
                send_to_char
                        ("God answers your prayers and gives you his vision.\n\r",
                         ch);
...
void do_steelskin(CHAR_DATA * ch, char *argument)
{
        ...
        {
                send_to_char
                        ("You have not learned the Steel of Skin Mantra.\n\r",
                         ch);
                return;
        }
...
void do_godsbless(CHAR_DATA * ch, char *argument)
{
        ...
        {
                send_to_char
                        ("You have not learned the Almighty Favor Mantra.\n\r",
                         ch);
                return;
        }
        if (ch->mana < 3000)
...
void do_wrathofgod(CHAR_DATA * ch, char *argument)
{
        ...
        {
                send_to_char
                        ("You have not learned to call on the wrath of god yet.\n\r",
                         ch);
                return;
        }
        if (ch->fighting != NULL)
...
void do_godsfavor(CHAR_DATA * ch, char *argument)
{
        ...
        {
                send_to_char
                        ("You have not learned the Almighty's Favor Mantra.\n\r",
                         ch);
                return;
        }
        if (ch->move < 1500)
        ...
        if (!IS_SET(ch->newbits, NEW_MONKFAVOR))
        {
                ...
                send_to_char
                        ("God smiles upon you and you feel strengthened.\n\r",
                         ch);
                act("$n is blessed by the Almighty.", ch, NULL, NULL,
                    TO_ROOM);
        ...
        if (IS_SET(ch->newbits, NEW_MONKFAVOR))
        {
                ...
                send_to_char
                        ("You feel weaker as The Almighty's favor leaves you.\n\r",
                         ch);
...
void do_darkblaze(CHAR_DATA * ch, char *argument)
{
        ...
        {
                send_to_char
                        ("You have not learned the Dark Blaze Mantra.\n\r",
                         ch);
                return;
        }
        if (ch->fighting != NULL)
...
void do_godsheal(CHAR_DATA * ch, char *argument)
{
        ...
        {
                send_to_char
                        ("You are surrounded in a glowing halo of energy.\n\r",
                         ch);
        ...
        {
                send_to_char
                        ("You feel a mighty force lay his hands on you.\n\r",
                         ch);
                act("$n is surrounded by a glowing halo, and looks very relaxed.\n\r", ch, NULL, NULL, TO_ROOM);
                ch->hit += 500;
                if (ch->hit > ch->max_hit)
...
void do_ghold(CHAR_DATA * ch, char *argument)
{
        ...
        {
                send_to_char
                        ("God joins you at your side to hold your foe.\n\r",
                         ch);
