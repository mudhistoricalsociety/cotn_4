...
void do_auction(CHAR_DATA * ch, char *argument)
{
        ...
        if (!str_cmp(arg1, "stop") && IS_IMMORTAL(ch))
        {
                ...
                {
                        stc("Nothing on auction.\n\r", ch);
                        return;
                }

                xprintf(buf,
                        "#R%s stops the auction. %s is confiscated.#n\n\r",
        ...
        if (!str_cmp(arg1, "info"))
        {
                ...
                {
                        send_to_char
                                ("There is nothing up for auction right now.\n\r",
                                 ch);
                        return;
                }
        ...
        if (!str_cmp(arg1, "bid"))
        {
                ...
                if (bah < bid)
                {
                        stc("You need to bid at least 500 more than the current bid.\n\r", ch);
                        return;
                }
                ...
                {
                        xprintf(buf,
                                "You must bid above the current bid of %ld Bones.\n\r",
                ...
                xprintf(buf, "#B%s #wjust bid #Y%ld #wbones for %s.#n\n\r",
                        ch->pcdata->switchname, bid,
                        auction_info.item->short_descr);
                do_auchn(ch, buf);
                if (auction_info.high_bidder != NULL)
                {
                        auction_info.high_bidder->bones +=
        ...
        if (arg2[0] == '\0')
        {
                stc("Syntax: auction <object> <min bid>", ch);
                return;
        }
...
/*    if ( obj->questowner != NULL && strlen(obj->questowner) > 1 )
    {    
       ...
       return;
        ...
        xprintf(buf,
                "%s has put up %s for auction. [Minimum Bid: %ld bones]\n\r",
...
void auction_update()
{
        ...
        {
                xprintf(buf,
                        "#w%s #R#fSOLD#n #wto %s for #Y%ld #0Bones.#n\n\r",
                        capitalize(auction_info.item->short_descr),
        ...
        if (auction_info.status == AUCTION_LENGTH - 2)
        {
                ...
                {
                        xprintf(buf,
                                "No bids on %s - item sent back to Auctioner.\n\r",
                                auction_info.item->short_descr);
                        do_auchn(auction_info.high_bidder, buf);
...
void show_obj_stats(CHAR_DATA * ch, OBJ_DATA * obj)
{
        ...
        case ITEM_WEAPON:
                {
                        ...
                        if (IS_SET(obj->spectype, SITEM_COPPER))
                                send_to_char
                                        ("This item is forged with copper.\n\r",
                                         ch);
                        if (IS_SET(obj->spectype, SITEM_IRON))
                                send_to_char
                                        ("This item is forged with iron.\n\r",
                                         ch);
                        if (IS_SET(obj->spectype, SITEM_STEEL))
                                send_to_char
                                        ("This item is forged with steel.\n\r",
                                         ch);
                        ...
                        if (IS_SET(obj->spectype, SITEM_DEMONIC))
                                send_to_char
                                        ("This item is crafted from demonsteel.\n\r",
                                         ch);
                        else if (IS_SET(obj->spectype, SITEM_SILVER))
                                send_to_char
                                        ("This item is crafted from gleaming silver.\n\r",
                                         ch);
                        ...
                        if (itemtype == 4)
                                ...
                                xprintf(buf,
                                        "This ancient weapon protects its wearer from player attacks.\n\r");
                        else if (itemtype == 20)
                                xprintf(buf,
                                        "This ancient weapon surrounds its wielder with a shield of darkness.\n\r");
                        else if (itemtype == 21)
                                xprintf(buf,
        ...
        case ITEM_ARMOR:
                ...
                if (obj->value[3] == 4)
                        ...
                        xprintf(buf,
                                "This powerful item protects its wearer from player attacks.\n\r");
                else if (obj->value[3] == 20)
                        xprintf(buf,
                                "This ancient item surrounds its wearer with a shield of darkness.\n\r");
