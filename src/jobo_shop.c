...
void do_obuy(CHAR_DATA * ch, char *argument)
{
        char      buf[MAX_STRING_LENGTH];
        CHAR_DATA *keeper;

        if (argument[0] != '\0')
                xprintf(buf, "buy %s", argument);
        else
                xprintf(buf, "buy");
...
void shopspec_generic(CHAR_DATA * keeper, CHAR_DATA * ch, char *argument)
{
        ...
        if (!str_cmp(arg, "list"))
        {
                ...
                {
                        do_say(keeper,
                               "I'm afraid I'm not selling anything right now");
                        return;
                }
                else
                {
                                ...
                                if (is_full_name(item, obj->name))
                                {
                                        ...
                                        obj = create_object(get_obj_index
                                                            (obj->pIndexData->
                                                             vnum), 50);
                                        obj_to_char(obj, ch);
                                        act("$N gives you $p.", ch, obj,
                                            keeper, TO_CHAR);
                                        act("$N gives $n $p.", ch, obj,
                                            keeper, TO_ROOM);
...
void shopspec_healer(CHAR_DATA * keeper, CHAR_DATA * ch, char *argument)
{
        ...
        {
                do_say(keeper,
                       "Welcome traveller, may I be of some service?");
        ...
        else if (!str_cmp(arg, "buy"))
        {
                ...
                if (!str_cmp(item, "healing"))
                {
                        ...
                        if (ch->move > ch->max_move)
                                ch->move = ch->max_move;
                        do_say(keeper,
                               "May you wounds heal and your heart grow strong");
                        ch->bones -= cost;
                        WAIT_STATE(ch, 6);
                }
                else if (!str_cmp(item, "darkbless"))
                {
                        ...
                        {
                                do_say(keeper,
                                       "Theres something wrong with my magic, please tell an immortal about this");
                                return;
                        }
