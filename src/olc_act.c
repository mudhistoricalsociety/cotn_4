...
const struct olc_help_type help_table[] = {
        ...
        {"aweather", aweather_flags, "Weather Dispositions"},
        {"room", room_flags, "Room attributes."},
        {"sector", sector_flags, "Sector types, terrain."},
        {"asector", asector_flags, "Area Sector types, terrain."},
...
bool redit_rlist(CHAR_DATA * ch, char *argument)
{
        ...
        for (vnum = pArea->lvnum; vnum <= pArea->uvnum; vnum++)
        {
                ...
                {
                        found = TRUE;
                        xprintf(buf, "[%6d] %-40s [%-10s]", vnum, pRoom->name,
...
bool redit_spell(CHAR_DATA * ch, char *argument)
{
        ...
        if (str_cmp(keyword, "create"))
        {
                ...
                {
                        send_to_char
                                ("No such spell, you'll need to create it first.\n\r",
                                 ch);
                        return FALSE;
                }
        }
        if (!str_cmp(keyword, "delete"))
        {
                ...
                else
                {
                        ...
                        for (rt_next = pRoom->roomtext; rt_next;
        ...
        else if (!str_cmp(keyword, "create"))
        {
                ...
                {
                        send_to_char
                                ("There is already a roomprogram with that keyword.\n\r",
                                 ch);
                        return FALSE;
                }
...
bool redit_action(CHAR_DATA * ch, char *argument)
{
        ...
        if (str_cmp(keyword, "create"))
        {
                ...
                {
                        send_to_char
                                ("No such action, you'll need to create it first.\n\r",
                                 ch);
                        return FALSE;
                }
        }
...
bool redit_texttrig(CHAR_DATA * ch, char *argument)
{
        ...
        if (str_cmp(keyword, "create"))
        {
                ...
                {
                        send_to_char
                                ("No such texttrigger, you'll need to create it first.\n\r",
                                 ch);
                        return FALSE;
                }
        }
...
bool redit_rprog(CHAR_DATA * ch, char *argument)
{
        ...
        if (str_cmp(keyword, "create"))
        {
                ...
                {
                        send_to_char
                                ("No such roomprogram, you'll need to create it first.\n\r",
                                 ch);
                        return FALSE;
                }
        }
...
bool redit_portal(CHAR_DATA * ch, char *argument)
{
        ...
        if (str_cmp(keyword, "create"))
        {
                ...
                {
                        send_to_char
                                ("No such portal, you'll need to create it first.\n\r",
                                 ch);
                        return FALSE;
                }
        }
        ...
        else if (!str_cmp(keyword, "create"))
        {
                ...
                rt->output =
                        str_dup
                        ("A shimmering portal appears in front of $n.");
                rt->choutput =
                        str_dup
                        ("A shimmering portal appears in front of you.");
                rt->name = str_dup("all");
                rt->type = RT_PORTAL;
                rt->power = 0;  /* Not set yet */
...
bool redit_teleport(CHAR_DATA * ch, char *argument)
{
        ...
        if (str_cmp(keyword, "create"))
        {
                ...
                {
                        send_to_char
                                ("No such teleport, you'll need to create it first.\n\r",
                                 ch);
                        return FALSE;
                }
        }
        ...
        else if (!str_cmp(keyword, "create"))
        {
                ...
                {
                        send_to_char
                                ("There is already a teleport with that keyword.\n\r",
                                 ch);
                        return FALSE;
                }
...
bool aedit_show(CHAR_DATA * ch, char *argument)
{
        ...
        xprintf(buf, "#wBits:     #0[#W%s#0]#n\n\r",
                flag_string(area_bits, pArea->areabits));
        send_to_char(buf, ch);

        xprintf(buf, "#wSector:   #0[#W%s#0]#n\n\r",
                flag_string(asector_flags, pArea->sector));
        send_to_char(buf, ch);

        xprintf(buf, "#wAllowed:  #0[#W%s#0](%d)#n\n\r",
                flag_string(aweather_flags, pArea->a_weather),
                pArea->a_weather);
        send_to_char(buf, ch);

        xprintf(buf, "#wWeather:  #0[#n%s#0]#n\n\r",
                flag_string(weather_flags, pArea->sky));
        send_to_char(buf, ch);
...
bool aedit_vnum(CHAR_DATA * ch, char *argument)
{
        ...
        if (get_vnum_area(ilower) && get_vnum_area(ilower) != pArea)
        {
                ...
                xprintf(buf,
                        "AEdit:  Lower vnum %d already assigned in area %s.\n\r",
                        ilower, pArea2->name);
                send_to_char(buf, ch);
                return FALSE;
        }
...
bool redit_show(CHAR_DATA * ch, char *argument)
{
        ...
        for (door = 0; door < MAX_DIR; door++)
        {
                ...
                if ((pexit = pRoom->exit[door]))
                {
                        ...
                        xprintf(buf,
                                "#R-#Y%-5s #wto #0[#W%5d#0] #wKey: #0[#R%5d#0]#n",
        ...
        xprintf(buf, "Shop Type:   [%d]\n\r", pRoom->shop_type);
        stc(buf, ch);
        xprintf(buf, "Items Sold:  [%5d] [%5d] [%5d] [%5d]\n\r"
                "             [%5d] [%5d] [%5d] [%5d]\n\r"
                "             [%5d] [%5d] [%5d] [%5d]\n\r"
                "             [%5d] [%5d] [%5d] [%5d]\n\r"
                "             [%5d] [%5d] [%5d] [%5d]\n\r"
                "             [%5d] [%5d] [%5d] [%5d]\n\r",
                pRoom->items_sold[1], pRoom->items_sold[2],
                pRoom->items_sold[3], pRoom->items_sold[4],
                pRoom->items_sold[5], pRoom->items_sold[6],
                pRoom->items_sold[7], pRoom->items_sold[8],
                pRoom->items_sold[9], pRoom->items_sold[10],
                pRoom->items_sold[11], pRoom->items_sold[12],
                pRoom->items_sold[13], pRoom->items_sold[14],
                pRoom->items_sold[15], pRoom->items_sold[16],
                pRoom->items_sold[17], pRoom->items_sold[18],
                pRoom->items_sold[19], pRoom->items_sold[20],
                pRoom->items_sold[21], pRoom->items_sold[22],
                pRoom->items_sold[23], pRoom->items_sold[24]);
        stc(buf, ch);
...
bool change_exit(CHAR_DATA * ch, char *argument, int door)
{
        ...
        if (!str_cmp(command, "dig"))
        {
                ...
                if (arg[0] == '\0')
                {
                        ...
                        if (newvnum > pArea->uvnum)
                        {
                                send_to_char
                                        ("Dig Error: No more free vnums in area.\n\r",
                                         ch);
                                return FALSE;
                        }

                        xprintf(digroom, "%d", newvnum);
                        redit_create(ch, digroom);
                        xprintf(buf, "link %s", digroom);
                        change_exit(ch, buf, door);
                        return TRUE;
                ...
                else
                {
                        ...
                        {
                                send_to_char
                                        ("Error: Argument must be numerical\n\r",
                                         ch);
                                return FALSE;
                        }
                        else
        ...
        if (!str_cmp(command, "remove"))
        {
                ...
                if (argument[0] == 'd'
                    && !str_prefix(argument, "description"))
                {
...
bool redit_ed(CHAR_DATA * ch, char *argument)
{
        ...
        if (!str_cmp(command, "type"))
        {
                ...
                if (keyword[0] == '\0')
                {
                        send_to_char("Syntax: ed type [keyword] [type]\n\r",
                                     ch);
                        send_to_char
                                ("Valid types : none, pull, press, touch and push.\n\r",
                                 ch);
                        return FALSE;
                }
                for (ed = pRoom->extra_descr; ed; ed = ed->next)
        ...
        if (!str_cmp(command, "action"))
        {
                ...
                {
                        send_to_char
                                ("Syntax: ed action [keyword] [action]\n\r",
                                 ch);
                        send_to_char
                                ("Valid types : none, teleport, spell, object and elevator.\n\r",
                                 ch);
                        return FALSE;
                }
                for (ed = pRoom->extra_descr; ed; ed = ed->next)
...
bool redit_shoptype(CHAR_DATA * ch, char *argument)
{
        ROOM_INDEX_DATA *pRoom;
        char      arg[MAX_INPUT_LENGTH];
        int       value;
        ...
        if (arg[0] == '\0')
        {
                stc("Syntax: shoptype [number]\n\r", ch);
                return FALSE;
        }
        value = atoi(arg);

        pRoom->shop_type = value;
        return TRUE;
}

bool redit_shopitem(CHAR_DATA * ch, char *argument)
{
        ROOM_INDEX_DATA *pRoom;
        OBJ_INDEX_DATA *pObjIndex;
        int       i, value, vnum;
        ...
        if (pRoom->shop_type == 0)
        {
                stc("This room is not a shop", ch);
                return FALSE;
        }

        if (arg[0] == '\0')
        {
                stc("Syntax: Shopitem [vnum]\n\r", ch);
                return FALSE;
        }
        ...
        if (!str_cmp(arg, "clear"))
        {
                for (i = 1; i < 25; i++)
                {
                        ...
                        pRoom->items_sold[i] = 0;
                }
                stc("All items cleared.\n\r", ch);
                return TRUE;
        }
        ...
        {
                stc("Sex is good, but that item isn't.", ch);
                return FALSE;
        }
        for (i = 1; i < 25; i++)
        {

                if (pRoom->items_sold[i] == value)
                {
                        //vnum = pRoom->items_sold[i];
                        //if ( ( pObjIndex = get_obj_index( vnum ) ) != NULL )
                        //{
                        sprintf(buf, "Item %s no longer sold at this store.",
                        ...
                        /*
                         * }
                         * else if ((pObjIndex = get_obj_index( vnum) ) == NULL )
                         * {
                         * stc("Error: Object not found.",ch);
                         * return FALSE;
                         * } 
                         */
                }

                else if (pRoom->items_sold[i] == 0
                         && (pObjIndex = get_obj_index(vnum)) != NULL)
                {
                        pRoom->items_sold[i] = value;
                        sprintf(buf, "Item %s now sold at this store.",
                                pObjIndex->short_descr);
                        stc(buf, ch);
                        return TRUE;
                }
...
const struct wear_type wear_table[] = {
        ...
        {WEAR_FAMILIAR, ITEM_WEAR_FAMILIAR},
        {NO_FLAG, NO_FLAG}
};
...
bool redit_oreset(CHAR_DATA * ch, char *argument)
{
        ...
        if ((to_mob = get_char_room(ch, NULL, arg2)) != NULL)
        {
                 ...
                 */
                if ((w_loc = flag_value(wear_loc_flags, argument)) == NO_FLAG)
                {
                        send_to_char
                 ...
                 */
                if (!IS_SET(pObjIndex->wear_flags, wear_bit(w_loc)))
                {
                        xprintf(output,
...
void show_obj_values(CHAR_DATA * ch, OBJ_INDEX_DATA * obj)
{
        ...
        case ITEM_WEAPON:
                xprintf(buf,
                        ...
                        obj->value[0],
                        obj->value[1],
                        obj->value[2], flag_string(weapon_flags,
                                                   obj->value[3]));
                send_to_char(buf, ch);
                break;
        case ITEM_ARMOR:
               ...
               char *argument)
{
        ...
        case ITEM_WINDOW:
                ...
                default:
                        send_to_char
                                ("set v0 to the destination window, or to zero if you wish to use the windows description \n\r",
                                 ch);
                        return FALSE;
                case 0:
        ...
        case ITEM_FAMILIAR:
                ...
                case 2:
                        ...
                        break;
                case 3:
                        send_to_char("Min Tier Level Set.\n\r\n\r", ch);
                        pObj->value[3] = atoi(argument);
                        break;
                case 4:
                ...
                case 5:
                        ...
                        break;
                case 6:
                        send_to_char("Learning Rate Set.\n\r\n\r", ch);
        ...
        case ITEM_SIGIL:
                ...
                {
                default:
                        do_help(ch, "ITEM_SIGIL");
                        return FALSE;
                case 0:
                        send_to_char("Attack amount set.\n\r\n\r", ch);
                        pObj->value[0] = atoi(argument);
                        break;
                case 1:
                        send_to_char("FireAttack amount set.\n\r\n\r", ch);
                        pObj->value[1] = atoi(argument);
                        break;
                case 2:
                        send_to_char("WaterAttack amount set.\n\r\n\r", ch);
                        pObj->value[2] = atoi(argument);
                        break;
                case 3:
                        send_to_char("LightningAttack amount set.\n\r\n\r",
                                     ch);
                        pObj->value[3] = atoi(argument);
                        break;
                case 4:
                        send_to_char("EarthAttack amount set.\n\r\n\r", ch);
                        pObj->value[4] = atoi(argument);
                        break;
                case 5:
                        send_to_char("Vitality amount set.\n\r\n\r", ch);
                        pObj->value[5] = atoi(argument);
                        break;
                case 6:
                        send_to_char("Hitroll amount set.\n\r\n\r", ch);
                        pObj->value[6] = atoi(argument);
                        break;
                case 7:
                        send_to_char("Damroll amount set.\n\r\n\r", ch);
                        pObj->value[7] = atoi(argument);
                        break;
                case 8:
                        send_to_char("ArmorClass amount set.\n\r\n\r", ch);
...
bool oedit_show(CHAR_DATA * ch, char *argument)
{
        ...
        for (cnt = 0, paf = pObj->affected; paf; paf = paf->next)
        {
                ...
                xprintf(buf, "#0[#Y%4d#0] #w%-8d #B%s#n\n\r", cnt,
                        paf->modifier, flag_string(apply_flags,
                                                   paf->location));
                send_to_char(buf, ch);
                cnt++;
        }

        if (pObj->item_type == ITEM_FAMILIAR)
        {
                sprintf(buf, "#R*#0----------------------#R*#n\n\r"
                        "#0|                      |#n\n\r"
                        "#0| #RSpec Fun#r[#R%-10d#r]#0 |#n\n\r"
                        "#0| #RLevel   #r[#R%-10d#r]#0 |#n\n\r"
                        "#0| #RBase Dmg#r[#R%-10d#r]#0 |#n\n\r"
                        "#0| #RMin Tier#r[#R%-10d#r]#0 |#n\n\r"
                        "#0| #RTo Hit  #r[#R%-10d#r]#0 |#n\n\r"
                        "#0| #RExp     #r[#R%-10d#r]#0 |#n\n\r"
                        "#0| #RLearning#r[#R%-10d#r]#0 |#n\n\r"
                        "#0| #RAlign   #r[#R%-10d#r]#0 |#n\n\r"
                        "#0| #RHit Msg1#r[#R%s#r]#0       |#n\n\r"
                        "#0| #RHit Msg2#r[#R%s#r]#0       |#n\n\r"
                        "#p|                      |#n\n\r"
                        "#P*#p----------------------#P*#n\n\r",
                        pObj->value[0], pObj->value[1], pObj->value[2],
                        pObj->value[3], pObj->value[4], pObj->value[5],
                        pObj->value[6], pObj->value[7], pObj->hit_msg1,
                        pObj->hit_msg2);
                stc(buf, ch);
        }
...
bool oedit_msg1(CHAR_DATA * ch, char *argument)
{
        OBJ_INDEX_DATA *pObj;
        ...
        if (argument[0] == '\0')
        {
                send_to_char("Syntax:  msg1 [string]\n\r", ch);
                return FALSE;
        }

        free_string(pObj->hit_msg1);
        pObj->hit_msg1 = str_dup(argument);

        send_to_char("Message1 set.\n\r", ch);
        return TRUE;
}

bool oedit_msg2(CHAR_DATA * ch, char *argument)
{
        OBJ_INDEX_DATA *pObj;
        ...
        if (argument[0] == '\0')
        {
                send_to_char("Syntax:  msg2 [string]\n\r", ch);
                return FALSE;
        }

        free_string(pObj->hit_msg2);
        pObj->hit_msg2 = str_dup(argument);

        send_to_char("Message2 set.\n\r", ch);
        return TRUE;
}
...
bool oedit_value10(CHAR_DATA * ch, char *argument)
{
...
bool oedit_condition(CHAR_DATA * ch, char *argument)
{
        ...
        {
                send_to_char
                        ("Syntax:  condition [1 - 100] (100 being best).\n\r",
                         ch);
                return FALSE;
        }
...
bool oedit_resistance(CHAR_DATA * ch, char *argument)
{
        ...
        {
                send_to_char
                        ("Syntax:  resistance [1 - 25] (1 being best) use 1 for relics.\n\r",
                         ch);
                return FALSE;
        }
...
bool oedit_toughness(CHAR_DATA * ch, char *argument)
{
        ...
        {
                send_to_char
                        ("Syntax:  toughness [5 - 100] (100 being best)\n\r",
                         ch);
                return FALSE;
        }
...
bool medit_show(CHAR_DATA * ch, char *argument)
{
        ...
        if (pMob->quest_fun)
        {
                xprintf(buf, "#wQuest fun:    #0[#W%s#0]\n\r",
                        quest_string(pMob->quest_fun));
                send_to_char(buf, ch);
        }
        if (pMob->shop_fun)
        {
                xprintf(buf, "#wShop fun:    #0[#W%s#0]\n\r",
                        shop_string(pMob->shop_fun));
                send_to_char(buf, ch);
        }
...
bool oedit_addoprog(CHAR_DATA * ch, char *argument)
{
        ...
        {
                send_to_char
                        ("Syntax:   addoprog [vnum] [trigger] [phrase]\n\r",
                         ch);
                return FALSE;
        }
...
bool redit_addrprog(CHAR_DATA * ch, char *argument)
{
        ...
        {
                send_to_char
                        ("Syntax:   addrprog [vnum] [trigger] [phrase]\n\r",
                         ch);
                return FALSE;
        }
...
HEDIT(hedit_change)
{
        ...
        for (tHelp = first_help; tHelp != NULL; tHelp = tHelp->next)
        {
                ...
                {
                        send_to_char
                                (" Help found, Entering String editor\n\r",
                                 ch);
